...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	// using range-based for loop available in C++11, recommended by Justin Joyce
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	const float center = 1.0 - blurring;
	const float diagonal = blurring / 12.0;
	const float adjacent = blurring / 6.0;

	vector < vector <float> > window{ { diagonal, adjacent, diagonal },
		...
		for (int j = 0; j < width; j++) {
			...
			for (int dx : {-1, 0, 1}) {
				for (int dy : {-1, 0, 1}) {
					...
					float multi_amt = window[dx + 1][dy + 1];
					newGrid[new_i][new_j] += cell_value * multi_amt;
				}
			}
		}
