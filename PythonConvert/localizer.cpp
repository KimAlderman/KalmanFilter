...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	int grid_width, grid_height, grid_area;
	grid_width = grid[0].size();
	grid_height = grid.size();
	grid_area = grid_width * grid_height;

	float norm_prob;
	norm_prob = 1.0 / grid_area;

	vector< vector <float> > newGrid(grid_height, vector<float>(grid_width, norm_prob));
	...
	float p_miss)
{
	...
	int grid_width, grid_height, i, j;
	grid_width = grid[0].size();
	grid_height = grid.size();
	...
	float blurring)
{
	...
	int width, height, i, j, newi, newj;
